...
import { Send, Bot, User2, Loader2, Sparkles } from "lucide-react"
...
const SAMPLE_QUESTIONS = [
  "What are the top vendors by transaction volume?",
  "Show me entities related to supplier risk",
  "Find all products with low inventory",
  "What relationships exist between vendors and products?",
]

const MOCK_RESPONSES: Record<string, string> = {
  default:
    "I've searched the knowledge graph and found relevant information. The graph contains multiple interconnected entities that relate to your query. Would you like me to drill deeper into any specific aspect?",
}

function getResponse(query: string): string {
  return MOCK_RESPONSES[query.toLowerCase()] ?? MOCK_RESPONSES.default
}
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "welcome",
      role: "assistant",
      content: `Hi${user?.name ? ` ${user.name}` : ""}! I'm your knowledge graph assistant. Ask me anything about the data you've ingested — entities, relationships, patterns, or trends.`,
      timestamp: new Date(),
    },
  ])
      ...
      <div className="flex items-center gap-3 border-b border-border px-5 py-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Powered by RKG · Graph-grounded responses
          </p>
        </div>
